...
class QueryBuilder:
    ...
    def __init__(self, measurement="", period="time >= '2019-01-01'", limit=25):
        self._limit = limit
    ...
    def generate_query(self):
        return self._select_clause() + self._where_clause() + self._limit_clause()
...
class InfluxService:
    ...
    def check_db():
        ...
        if InfluxService.cnf.INFLUX_DB not in [str(x['name']) for x in all_dbs_list]:
            try:
                app_logger.warning("Database {0} does not exist".format(InfluxService.cnf.INFLUX_DB))
            ...
            try:
                app_logger.info("Using db {0}".format(InfluxService.cnf.INFLUX_DB))
                InfluxService.db_client.switch_database(InfluxService.cnf.INFLUX_DB)
            except exceptions.InfluxDBClientError as e:
                app_logger.error(str(e))
            except exceptions.InfluxDBServerError as e1:
                app_logger.error(str(e1))
